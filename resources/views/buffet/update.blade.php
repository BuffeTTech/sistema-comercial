                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('buffet.update', ['buffet'=>$buffet->slug]) }}">
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <x-input-label for="buffet_logo" :value="__('Logo*')" class="dark:text-slate-100 text-slate-800"/>
                                <x-text-input id="buffet_logo" class="block mt-1 w-full dark:bg-slate-100 dark:text-slate-500" type="file" name="buffet_logo" required autofocus />
                                <!-- adicionar o preview -->
                                <div>
                                    <img src="" alt="">
                                </div>
                                <x-input-error :messages="$errors->get('buffet_logo')" class="mt-2" />
                            </div>
                            <h3 class="text-md font-semibold mb-1 mt-1">Horario de funcionamento</h3>
                            @foreach($buffet_schedules as $schedule)
                                <div class="mt-2 justify-center items-center flex-row">
                                    <span>{{ App\Enums\DayWeek::getEnumByName($schedule->day_week)}}</span>
                                    <span><span class="{{$schedule->opened ? 'bg-blue-100 text-black' : ''}}">Aberto</span> | <span class="{{!$schedule->opened ? 'bg-blue-100 text-black' : ''}}">Fechado</span></span>
                                    <div class="inline-block {{!$schedule->opened ? 'bg-gray-500' : ''}}">
                                        <label for="buffet_schedule_{{$schedule->day_week}}_start">Das</label>
                                        <input type="time" name="buffet_schedule[{{$schedule->day_week}}][start]" id="buffet_schedule_{{$schedule->day_week}}_start" {{!$schedule->opened ? 'disabled' : ''}} value="{{$schedule->start}}" class="text-slate-800">
                                        <label for="buffet_schedule_{{$schedule->day_week}}_end">Até</label>
                                        <input type="time" name="buffet_schedule[{{$schedule->day_week}}][end]" id="buffet_schedule_{{$schedule->day_week}}_end" {{!$schedule->opened ? 'disabled' : ''}} value="{{$schedule->end}}" class="text-slate-800">
                                    </div>
