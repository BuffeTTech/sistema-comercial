        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div>
                        <form method="POST" action="{{ route('survey.update', ['buffet'=>$buffet->slug, 'survey'=>$survey->hashed_id]) }}" enctype="multipart/form-data" id="form">
                            @method('put')
                            @csrf
                            ...
                            <div class="flex flex-wrap -mx-3 mb-6">
                                <div class="w-full  px-3 mb-6 md:mb-0">
                                    ...
                                    <textarea name="question" id="question" cols="40" rows="10" class="height-500 width-500" placeholder="Insira a questão">{{old('question') ?? $survey->question}}</textarea>
                                </div>
                            </div>
                                ...
                                <div class="w-full  px-3 mb-6 md:mb-0">                            
                                    ...
                                    <select name="question_type" id="question_type" required>
                                        ...
                                        @foreach( App\Enums\QuestionType::array() as $key => $value )
                                            <option value="{{$value}}" {{ $survey->question_type == $value ? 'selected' : ""}}>{{$key}}</option>
                                        @endforeach
                                    </select>
