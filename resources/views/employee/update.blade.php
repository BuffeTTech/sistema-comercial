                ...
                <div>
                    <form method="POST" action="{{ route('employee.update', ['buffet'=>$buffet->slug, 'employee'=>$employee->hashed_id]) }}" id="form">
                        ...
                        <div>
                            ...
                            <select name="document_type" id="document_type">
                                @foreach( App\Enums\DocumentType::array() as $key => $value )
                                    <option value="{{$value}}" {{ $employee->document_type == $value ? 'selected' : ""}}>{{$key}}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="document" class="block mt-1 w-full" type="text" name="document" :value="old('document') ?? $employee->document" required autofocus autocomplete="document" />
                        ...
                        <div>
                            ...
                            <x-text-input id="phone1" class="block mt-1 w-full phone" type="text" name="phone1" :value="old('phone1') ?? $employee->user_phone1->number" required autofocus autocomplete="phone1" />
                        ...
                        <div>
                            ...
                            <select name="role" id="role">
                                ...
                                @foreach($roles as $role)
                                    <option 
                                        {{ $employee->roles[0]->name == $role['name'] || old('role') == $role['name'] ? "selected" : ""}}
                                        value="{{ $role->name }}">{{ ucwords(explode($slug.'.', $role->name)[1]) }}</option>
                                @endforeach
                            </select>
