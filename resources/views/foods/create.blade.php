    ...
    <div>
        <form method="POST" action="{{ route('food.store', ['buffet'=>$buffet->slug]) }}" enctype="multipart/form-data">
            ...
            <div>
                ...
                <x-text-input id="name_food" class="block mt-1 w-full" type="text" name="name_food" :value="old('name_food')" required autofocus />
            ...
            <x-input-error :messages="$errors->get('foods_photo_generic')" class="mt-2" />
            @for($i=0; $i<$configurations->max_food_photos; $i++)
                <div>
                    <x-input-label for="foods_photo{{ $i+1 }}" :value="__('Inserir Imagem')" />
                    <x-text-input id="foods_photo{{ $i+1 }}" class="block mt-1 w-full" accept="image/png, image/gif, image/jpeg" type="file" name="foods_photo[]" :value="old('foods_photo{{ $i+1 }}')" required autofocus />
                    <x-input-error :messages="$errors->get('foods_photo[{{ $i }}]')" class="mt-2" />
                </div> 
            @endfor
